...
import {
  AlchemySimulationReq,
  AlchemySimulationResp,
  DappsResponse,
  DefipulseResponse,
  Dictionary,
  DoHResp,
...
async function alchemySimulate(
  simData: AlchemySimulationReq,
): Promise<AlchemySimulationResp | false> {
  try {
...
async function processTxs(network: supportedNetworkIds, req: FastifyRequest) {
  ...
  if (Object.keys(deserializedTxParsed).includes("to") === true) {
    let { isMalicious, rpcResp } = await checkAddress(
      deserializedTxParsed.to as string,
    );
  ...
  let gas_price =
    parseInt(deserializedTxParsed.maxFeePerGas || "0") +
    parseInt(deserializedTx.maxPriorityFeePerGas.toString(10) || "0");
  // Simulate Txn
  ...
  if (
    alResp != undefined &&
    alResp != false &&
    "transaction_info" in alResp &&
    alResp.transaction_info != undefined
  ) {
  ...
  if (
    network != "manual" &&
    "blockUnverifiedContracts" in query &&
    query.blockUnverifiedContracts === "true"
  ) {
    let verificationReq = await fetch(
      `https://sourcify.dev/server/check-all-by-addresses?addresses=${deserializedTxParsed?.to}&chainIds=1,4,11155111,137,80001,10,42161,421611`,
    );
  ...
  // test enableScanners
  if (
    network in ["mainnet", "polygon", "polygon-testnet"] &&
    query?.enableScanners !== undefined &&
    deserializedTxParsed.to !== undefined
  ) {
  ...
  if (
    query?.blockRecentDnsUpdates != undefined &&
    Boolean(parseInt(query.blockRecentDnsUpdates)) === true
  ) {
    let days = parseInt(query.blockRecentDnsUpdates);
    let dnsQuery = await fetch(
      `https://dns.google/resolve?name=${req.hostname}&type=SOA`,
    );
    if (dnsQuery.ok === true) {
      const dnsQueryResp = (await dnsQuery.json()) as DoHResp;
      if (
        dnsQueryResp.Answer.length > 0 &&
        parseSerialFromAnswerData(dnsQueryResp.Answer[0].data) != false
      ) {
        let dtUpdated = parseSerialFromAnswerData(dnsQueryResp.Answer[0].data);
        if (dtUpdated !== false) {
          let diffDays =
            (Date.now() - dtUpdated.valueOf()) / (60 * 60 * 24 * 1000);
          if (diffDays <= days) {
            let { rpcResp } = getMalRpcError(
              "The domain's DNS has been changed recently.",
            );
            return rpcResp;
          }
        }
  ...
  if (
    network === "mainnet" &&
    query?.useGasHawk != undefined &&
    query.useGasHawk === "true"
  ) {
...
server.get("/", function (req, reply) {
  reply.header(
    ...
    "default-src *; style-src 'self' 'unsafe-inline' cdnjs.cloudflare.com fonts.googleapis.com; script-src 'self' 'unsafe-inline'; img-src data: mintlify.s3-us-west-1.amazonaws.com ;",
  );
  return reply.sendFile("index.html");
});
server.get("/new", function (req, reply) {
  reply.header(
    "Content-Security-Policy",
    "default-src *; style-src 'self' 'unsafe-inline' cdnjs.cloudflare.com fonts.googleapis.com; script-src 'self' 'unsafe-inline'; img-src data: mintlify.s3-us-west-1.amazonaws.com ;",
  );
...
server.get("/blacklist/*", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  if (res === undefined) return reply.send({ blacklisted: false });
    ...
    return reply.send({
      blacklisted: true,
      list: numToBlacklist[res],
      closestWhitelisted: { url: closestValid, distance: closestScore },
    });
  }
});
...
server.get(
  ...
  async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    for (let i = 0; i < tokenList.length; i++) {
      if (
        Boolean(tokenList[i]?.address) &&
        tokenList[i]?.address.toLowerCase() === params?.ethAddress.toLowerCase()
      ) {
...
server.post(
  ...
  async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (supportedJackets.includes(jacket)) {
      const { network, address } = req.body as {
        network: string;
        address: string;
      };
      if (network != undefined && address != undefined && isAddress(address)) {
        if (jacket === "slither") {
          let sr = await testUsingSlither(
            network as lifejacketSupportedNetwork,
            address,
          );
        ...
        return reply.send({
          success: false,
          error: "Invalid body params, network or address",
        });
    } else
      return reply.send({
        success: false,
        error: `Invalid LifeJacket, supported ${supportedJackets.toString()}`,
      });
  },
);
...
server.post("/:network", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  if (!isPhishing && Boolean(hashTable.get(hostname)) === false) {
    ...
    if (network && netIdToEnv.get(network)) {
      ...
      return reply.send({
        error: `Invalid network '${network}', available networks are ${Array.from(netIdToEnv.keys()).join(", ")}`,
      });
    }
  } else {
    let { rpcResp } = getMalRpcError(
      `🚨 Phishing detector for the site ${hostname} has been triggered.`,
    );
...
server.addHook("preHandler", (req, reply, done) => {
  reply.header(
    "onion-location",
    `http://omnid2kq4qppsp2a5hgeqok6ahkfo23tdtoe7km4uier4o43taxct5ad.onion${req.url}`,
  );
  done();
});
...
async function clearWhitelist() {
  ...
  let defillamaList: { protocols: Array<{ url: string }> } = await fetch(
  ...
  let defipulselist: DefipulseResponse = await fetch(
    "https://gist.githubusercontent.com/anudit/8df081a368397dea5ff4ce8bdfac6256/raw/9d36d984fe3ed78bd1f993c4cca22ec4093d9253/index.json",
  ).then((r) => r.json());
  ...
  let walletconnectlist: Array<WalletconnectResponse> = await fetch(
    "https://gist.githubusercontent.com/anudit/a88c5888dcfb7b9cbf14a57d8eca61ad/raw/c9e02047a31989715249c9251e828fbab15d8140/links.json",
  ).then((r) => r.json());
...
async function compileBlacklist() {
  ...
  const logStat = (tag: blacklistIndices, len = 0): Dictionary<any> => {
    ...
    let diff = htlen - prev;
    let ret = {
      Name: numToBlacklist[tag].id,
      Length: len,
      "Unique Added": `${diff.toLocaleString()} [+${((diff / len) * 100).toFixed(2)}%]`,
      Total: htlen,
    };
    prev = htlen;
    return ret;
  ...
  let blacklist1: Array<string> = await fetch(
  ...
  let blacklist2: { blacklist: Array<string> } = await fetch(
  ...
  let blacklist3: Array<{ id: string }> = await fetch(
  ...
  let blacklist4: Array<string> = await fetch(
    "https://raw.githubusercontent.com/DAOBuidler/MetaShieldExtension/main/function/data/domain_blacklist.json",
  ).then((r) => r.json());
  ...
  try {
    let blacklist5: { result: Array<string> } = await fetch(
  ...
  let blacklist7 = await fetch(
    "https://gist.githubusercontent.com/anudit/643a5a5a7b105a563836578fa6dfdbd1/raw/964215961af0bdce8e64a112da52a022f4679cf7/Chainabuse-ScamDomains.json",
  );
  ...
  let blacklist9 = await fetch("https://phish.sinking.yachts/v2/all").then(
    (r) => r.json(),
  );
  ...
  let blacklist11: Array<string> = await fetch(
  ...
  let blacklist12: Array<string> = await fetch(
  ...
  let blacklist13: { data: Array<{ scams: Array<{ url: string }> }> } =
    await fetch(
      "https://raw.githubusercontent.com/blueshell-io/scamdb/bec21924d47b3391196b7bb701316a75a3ac6009/website.json",
    ).then((e) => e.json());
  let res13 = blacklist13.data
    .map((e) => e.scams)
    .flat()
    .filter((e) => e != null)
    .map((d) => d.url)
...
server.listen(
  { port: parseInt(getEnv("PORT")) || 5003, host: "0.0.0.0" },
  (err, address) => {
    rpcSet();
    compileBlacklist();
